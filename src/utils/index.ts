 ...
 * 对象项转换后的数组元素接口
 * @interface
 * @property {string} name - 对象属性名
 * @property {string|number|boolean|Array<string>} value - 对象属性值
...
/**
 * 将对象转换为特定格式的数组
 * @param {Record<string, any>} obj - 待转换的原始对象
 ...
 * // 输出: [{ name: 'selectedDictCode', value: [] }]
 */
 ...
 * // 输出: { key: 'value', num: 123 }
 */
export const convertArrayToObject = (
  arr: Array<{ name: string; value: any }>
): Record<string, any> => {
  return arr.reduce((acc, item) => {
    // 处理特殊情况：selectedDictCode 空数组转回空字符串
    if (item.name === 'selectedDictCode' && Array.isArray(item.value) && item.value.length === 0) {
      acc[item.name] = '';
    } else {
