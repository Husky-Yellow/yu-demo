        ...
        <el-form label-position="top">
          ...
          <template v-if="selectedField.linkage.enabled">
            <el-form-item label="选择联动字段">
              <el-select
                v-model="selectedField.linkage.targetFieldId"
...
const otherFields = computed(() => {
  ...
  const allFormFields = formRows.value
    ...
    .filter((field): field is FormField => 'type' in field && field.type !== 'placeholder')
...
const isRowSelected = (row: FormRow): boolean => {
  return !!(selectedField.value && row.fields.some((f) => f.id === selectedField.value.id))
}
